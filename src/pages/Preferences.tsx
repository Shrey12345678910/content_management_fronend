...
export default function Preferences() {
  ...
  if (error && !prefs) {
    return <p className="text-red-600 text-sm">Couldn't load preferences: {error}</p>;
  }
  ...
  return (
    <div className="max-w-xl">
      ...
      <p className="text-slate-500 mb-6">Customize how the AI assistant behaves</p>

      <div className="bg-white border border-slate-200 rounded-xl p-6 flex flex-col gap-4">
        <div>
          <label className="text-sm font-medium text-slate-700 mb-1 block">Preferred AI Model</label>
          <select
            value={prefs.model}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setPrefs({ ...prefs, creativity: Number(e.target.value) })}
            className="w-full"
          />
        </div>

        <div>
          <label className="text-sm font-medium text-slate-700 mb-1 block">Writing Style</label>
          <input
            value={prefs.style}
        ...
        <div>
          <label className="text-sm font-medium text-slate-700 mb-1 block">Custom System Prompt</label>
          <textarea
